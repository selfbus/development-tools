...
public class VariableValueCellRenderer extends DefaultTableCellRenderer
{
   ...
   protected String telegramValueStr(Variable var)
   {
      try
      {
         ...
         {
            seq = "(" + I18n.getMessage("BusMonitorCellRenderer.Sequence") + " "
               + Integer.toString(telegram.getSequence()) + ")";
         }
         else seq = "";

         final StringBuffer sb = new StringBuffer();
         (new MessageFormat(msgTelegram)).format(new Object[] { fromStr, destStr, appData, seq }, sb, null);
         return sb.toString();
      }
      catch (IOException e)
      {
